...
def doKMeans(data, clusters=0):
  ...
  model=KMeans(n_clusters=clusters,random_state=100).fit(data)
  return model
...
user1=user1[(timedelta(hours=17) >= user1.CallTime)]
#
...
#import collections
#collections.Counter.items(result)
#
#from collections import Counter
#words = ['a', 'b', 'c', 'a']
#Counter(result).keys() # equals to list(set(words))
#Counter(result).values() # coun
#
...
ax.scatter(model.cluster_centers_[:,1], model.cluster_centers_[:,0], s=169, c=['r','b','y','g'], marker='x', alpha=0.8, linewidths=2)
#
