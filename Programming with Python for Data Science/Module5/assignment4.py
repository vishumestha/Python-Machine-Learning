...
#
# .. your code here ..
df=pd.read_csv("D:/Data Science with Python/6.Programming with Python for Data Science/DAT210x-master/DAT210x-master/Module5/Datasets/Wholesale customers data.csv")
df=df.dropna(axis=0) # Remove the null values
...
#
# .. your code here ..
df=df.drop(axis=1,labels=['Channel','Region'])
...
for  n_clusters in cluster:
    centroids, labels,inertia_ = doKMeans(T, n_clusters)
    x.append(n_clusters)
    y.append(inertia_)
plt.scatter(x=x,y=y)
plt.show()
