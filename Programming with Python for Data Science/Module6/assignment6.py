...
X=pd.read_csv("D:/Data Science with Python/6.Programming with Python for Data Science/DAT210x-master/DAT210x-master/Cached Datasets/dataset-har-PUC-Rio-ugulino/dataset-har-PUC-Rio-ugulino.csv",delimiter=';',decimal=',')
print(X.isnull().sum())
from sklearn.preprocessing import LabelEncoder
from sklearn.preprocessing import OneHotEncoder
le=LabelEncoder()
categort_mask=(X.dtypes=="object")

#X=X.iloc[:100,:2]
categorical_col=X.columns[X.dtypes=="object"].tolist()
X[categorical_col]=X[categorical_col].apply(lambda x: le.fit_transform(x))
ohe=OneHotEncoder(categorical_features=categort_mask,sparse=False)
X_Oh=ohe.fit_transform(X)

from sklearn.feature_extraction import DictVectorizer

dv=DictVectorizer(sparse=False)
X_di=X.to_dict("records")
X_dec=dv.fit_transform(X_di)
